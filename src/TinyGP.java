...
public class TinyGP {
	...
	void setup_fitness(String fname) {
		try {
			...
			if(varnumber + randomnumber >= FSET_START)
				System.out.println("Too many vars and constants");
			...
			System.out.println("ERROR: Cannot load " + fname);
			System.exit(0);
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	int grow(char[] buffer, int pos, int max, int depth) {
		...
		if(pos == 0) //first node must be a function
			prim = 1;

		if(prim == 0 || depth == 0) {  //pick a constant or random number to include
			prim = (char) rd.nextInt(varnumber + randomnumber);
			buffer[pos] = prim;
			return pos + 1;
		}
		else { //otherwise, include a function and recurse twice to fill its value set.
			...
			switch(prim) {
				...
				case DIV:
					buffer[pos] = prim;
					return( grow(buffer, grow(buffer, pos+1, max, depth-1),
					 		max, depth-1));
			}
		}
		return 0; //should never get here
						...
						case DIV:
							parentcopy[mutsite] =
								(char) (rd.nextInt(FSET_END - FSET_START + 1)
									   + FSET_START);
				}	}  //preserving line numbers from source
			}
		}
		return parentcopy;
	}
